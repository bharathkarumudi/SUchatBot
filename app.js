...
bot.dialog('updateProfile', [ function (session, args) {
  ...
  else if (profile_field == 'first name' || profile_field == 'last name' || profile_field == 'name'){
    session.send(`Sorry, you cannot update your ${profile_field}`);
    session.endDialog();
  }
  else {
  ...
  queryDatabase(`SELECT COUNT(*) FROM StudentEnrolledCourses WHERE SUID=${suid}`, function(enrolledCount_cb) {
    if (enrolledCount_cb == 0) {
      ...
      if (schedule_field == 'class') {
       ...
       queryDatabase(`select concat(courseTitle, ' on ', convert(varchar(255), examSchedule, 121) ) from AvailableCourses where CourseID in (select CourseID from StudentEnrolledCourses where suid=${suid})`, function(examSchedule) {
         session.send(examSchedule);
       }); // end of query
     } // end of else if
   } // end of else
 });
